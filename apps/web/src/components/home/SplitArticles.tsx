...
export function SplitArticles({ articles }: { articles: ArticleWithAuthor[] }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-0">
      {leftArticle ? (
        <Link href={`/article/${leftArticle.slug}`} className="group/article lg:pr-8 flex flex-col h-full border-b-2 lg:border-b-0 border-gray-300 dark:border-news-border pb-6 lg:pb-0 hover:translate-y-[-2px] transition-all duration-300">
      ...
      {rightArticle ? (
        <Link href={`/article/${rightArticle.slug}`} className="group/article lg:pl-8 lg:border-l-2 border-gray-300 dark:border-news-border flex flex-col h-full pt-2 lg:pt-0 hover:translate-y-[-2px] transition-all duration-300">
          <article className="flex flex-col h-full">
            <h3 className="font-playfair font-bold text-[19px] leading-tight mb-2 line-clamp-3 group-hover/article:text-red-600 dark:group-hover/article:text-news-accent transition-colors">
              {rightArticle.title}
            </h3>
