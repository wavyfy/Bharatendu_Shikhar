...
export function ArticleMeta({ article, isArticlePage }: { article: ArticleWithAuthor, isArticlePage?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-start gap-3 w-full text-[11px] text-gray-500 dark:text-news-text-muted font-bold tracking-widest uppercase whitespace-nowrap overflow-hidden text-ellipsis">
      <div className="flex items-center gap-2">
        {badges.map((b, i) => (
          <span 
            key={i} 
            className="text-white dark:text-news-accent font-black border px-1 rounded shadow-sm"
